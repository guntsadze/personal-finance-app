...
export async function getPots(): Promise<Pot[]> {
  const { data } = await apiClient.get<Pot[]>("/pots");
  return data;
}
...
export async function addMoneyToPot(id: string, payload: MoneyPayload): Promise<Pot> {
  const { data } = await apiClient.post<Pot>(`/pots/${id}/add-money`, payload);
  return data;
}

export async function withdrawFromPot(id: string, payload: MoneyPayload): Promise<Pot> {
