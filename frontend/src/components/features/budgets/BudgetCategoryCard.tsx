...
export function BudgetCategoryCard({ budget, onEdit, onDelete }: BudgetCategoryCardProps) {
  ...
  return (
    <div className="bg-white rounded-2xl p-300 flex flex-col gap-300">
      ...
      <p className="text-preset-4 text-grey-500">Maximum of {formatCurrency(maximum)}</p>

      <ProgressBar color={color} percentage={percentage} />

      <div className="grid grid-cols-2 gap-200">
        <div className="flex flex-col gap-50 border-l-4 pl-200" style={{ borderLeftColor: `var(--color-${color})` }}>
          <span className="text-preset-5 text-grey-500">Spent</span>
          <span className="text-preset-4-bold text-grey-900">{formatCurrency(spent)}</span>
        </div>
        <div className="flex flex-col gap-50 border-l-4 border-beige-100 pl-200">
          ...
          <span className="text-preset-4-bold text-grey-900">{formatCurrency(remaining)}</span>
        </div>
      </div>
